...
interface GamificationProfile {
  points: { total: number; lifetime: number; earnedThisWeek: number };
  level: { current: number; title: string };
  streaks: { current: number; longest: number };
  progress: { progressPercentage: number; pointsForNextLevel: number };
  stats: { totalSessions: number; coursesCompleted: number };
  ranking: { global: number };
  nearbyLeaderboard?: any[];
}

interface Badge {
  badge: { name: string; icon?: string; color?: string; tier: string };
  earnedAt: string;
}
...
const GamificationDashboard: React.FC = () => {
  ...
  const fetchLeaderboard = async (type: string) => {
    try {
      const res = await api.get(`/api/gamification/leaderboard?type=${type}&limit=20`);
      if (res.data.success) {
            ...
            {activeTab === 'leaderboard' && (
              <div className="space-y-6">
                <div className="flex bg-gray-50 p-1 rounded-xl">
                  {[
                    { id: 'global', label: 'All Time', icon: Crown },
