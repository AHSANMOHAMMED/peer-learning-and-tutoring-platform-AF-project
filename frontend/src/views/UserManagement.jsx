  ...
  return (
    <Layout userRole="admin">
      <div className="max-w-[1400px] mx-auto w-full font-sans pb-12">
        ...
        {/* Header Region */}
        <div className="mb-8 flex flex-col md:flex-row justify-between items-start md:items-end gap-6 bg-slate-900 rounded-3xl p-8 shadow-2xl relative overflow-hidden">
           <div className="absolute top-0 right-0 w-[400px] h-[400px] bg-indigo-500/20 blur-[100px] rounded-full translate-x-1/3 -translate-y-1/3 pointer-events-none" />
           ...
           <div className="relative z-10">
              ...
              <p className="text-slate-400 font-medium text-sm">Manage platform identities, configure roles, and audit user activity.</p>
           </div>
           
           <div className="flex gap-4 relative z-10 w-full md:w-auto">
              <button onClick={() => fetchUsers({limit: 100})} className="flex-1 md:flex-none justify-center bg-slate-800 border border-slate-700 text-slate-300 hover:text-white hover:bg-slate-700 px-5 py-3 rounded-xl font-bold text-sm shadow-lg transition-all flex items-center gap-2">
                 <RefreshCw size={16} className={loading && !isViewModalOpen ? "animate-spin" : ""} /> Refresh
              </button>
              <button onClick={() => {
                setFormData({ username: '', email: '', password: '', role: 'student', district: '', stream: '', grade: '', firstName: '', lastName: '' });
                setIsCreateModalOpen(true);
              }} className="flex-1 md:flex-none justify-center bg-indigo-600 hover:bg-indigo-500 text-white px-5 py-3 rounded-xl font-bold text-sm shadow-lg shadow-indigo-500/20 transition-all flex items-center gap-2">
              ...
              <table className="w-full text-left border-collapse">
                 <thead>
                    <tr className="bg-slate-50 border-b border-slate-200">
                       <th className="p-5 pl-8 text-xs font-extrabold uppercase tracking-widest text-slate-500 whitespace-nowrap">Identity</th>
                       <th className="p-5 text-xs font-extrabold uppercase tracking-widest text-slate-500 whitespace-nowrap">Role Status</th>
                       <th className="p-5 text-xs font-extrabold uppercase tracking-widest text-slate-500 whitespace-nowrap hidden lg:table-cell">Institution</th>
                       <th className="p-5 text-xs font-extrabold uppercase tracking-widest text-slate-500 whitespace-nowrap hidden lg:table-cell">Region / Data</th>
                       <th className="p-5 text-xs font-extrabold uppercase tracking-widest text-slate-500 whitespace-nowrap hidden xl:table-cell">System Status</th>
                       <th className="p-5 pr-8 text-xs font-extrabold uppercase tracking-widest text-slate-500 whitespace-nowrap text-right">Actions</th>
                    </tr>
                 </thead>
                 <tbody>
                    <AnimatePresence>
                       {loading && !isViewModalOpen ? (
                          <tr>
                             <td colSpan="5" className="py-24 text-center">
                                ...
                                <p className="text-slate-500 font-bold">Querying directory...</p>
                             </td>
                          </tr>
                          ...
                          <tr>
                             <td colSpan="5" className="py-32 text-center">
                                ...
                                <h3 className="text-xl font-bold text-slate-800 mb-2">No identities found</h3>
                          ...
                          filteredUsers.map((user) => (
                             ...
                             >
                                ...
                                <td className="p-5">
                                   <span className={cn(
                                     "px-3 py-1 rounded-md text-xs font-extrabold uppercase tracking-widest inline-block border",
                                ...
                                <td className="p-5 hidden lg:table-cell">
                                   {user.school ? (
                                      <div>
                                         <p className="font-bold text-slate-800 text-sm whitespace-nowrap">{typeof user.school === 'object' ? user.school.name : 'Linked Institutional'}</p>
                                         <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">{typeof user.school === 'object' ? user.school.code : 'VALID-CODE'}</p>
                                      </div>
                                   ) : (
                                      <span className="text-slate-400 italic text-xs">Global Identity</span>
                                   )}
                                </td>
                                ...
                                <td className="p-5 pr-8 text-right">
                                   <div className="flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                      ...
                                      <button onClick={() => openEditModal(user)} className="p-2 text-slate-400 hover:text-amber-600 hover:bg-amber-50 border border-transparent hover:border-amber-100 rounded-xl transition-all" title="Edit Identity">
                                         <Edit2 size={18} />
                                      </button>
                                      <button onClick={() => handleDeleteUser(user)} className="p-2 text-slate-400 hover:text-rose-600 hover:bg-rose-50 border border-transparent hover:border-rose-100 rounded-xl transition-all" title="Terminate Identity">
                                         <Trash2 size={18} />
                                      </button>
                                   </div>
                                </td>
                    ...
                    <div className="p-6 md:p-8 bg-slate-50 border-b border-slate-100 flex justify-between items-center">
                       <div>
                          <h2 className="text-2xl font-extrabold text-slate-800">
                             {isCreateModalOpen ? 'Provision New Identity' : 'Modify Access Profile'}
                          </h2>
                          <p className="text-slate-500 font-medium text-sm mt-1">
                             {isCreateModalOpen ? 'Create a fully verified system user.' : `Updating records for ${selectedUser?.username}`}
                          </p>
                       </div>
                    ...
                    <form onSubmit={isCreateModalOpen ? handleCreateUser : handleUpdateUser} className="p-6 md:p-8 space-y-6">
                       ...
                       <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                          <div className="space-y-2">
                             <label className="text-xs font-bold text-slate-600 uppercase tracking-widest pl-1">System Username</label>
                          ...
                          <div className="space-y-2">
                             <label className="text-xs font-bold text-slate-600 uppercase tracking-widest pl-1">Primary Email</label>
                          ...
                          {isCreateModalOpen && (
                             <div className="space-y-2">
                                <label className="text-xs font-bold text-slate-600 uppercase tracking-widest pl-1">Initial Password</label>
                          ...
                          <div className="space-y-2">
                             <label className="text-xs font-bold text-slate-600 uppercase tracking-widest pl-1">Provisioning Role</label>
                          ...
                          <div className="space-y-2">
                             <label className="text-xs font-bold text-slate-600 uppercase tracking-widest pl-1">First Name</label>
                          ...
                          <div className="space-y-2">
                             <label className="text-xs font-bold text-slate-600 uppercase tracking-widest pl-1">Last Name</label>
                          ...
                          {(formData.role === 'student' || formData.role === 'tutor') && (
                             <>
                                <div className="space-y-2">
                                   <label className="text-xs font-bold text-slate-600 uppercase tracking-widest pl-1">District</label>
                                ...
                                <div className="space-y-2">
                                   <label className="text-xs font-bold text-slate-600 uppercase tracking-widest pl-1">Academic Stream</label>
                                ...
                                <div className="space-y-2">
                                   <label className="text-xs font-bold text-slate-600 uppercase tracking-widest pl-1">Linked Institution</label>
                       ...
                       <div className="flex gap-4 pt-4 border-t border-slate-100">
                          ...
                          <button type="submit" disabled={loading} className="flex-1 bg-indigo-600 text-white font-bold rounded-xl shadow-lg shadow-indigo-600/20 hover:bg-indigo-500 transition-all active:scale-95 disabled:opacity-70 flex justify-center items-center">
                             {loading ? <RefreshCw className="animate-spin" size={20} /> : (isCreateModalOpen ? 'Provision Identity' : 'Save Changes')}
                          </button>
                       </div>
                    </form>
                 </motion.div>
              ...
              <div className="fixed inset-0 z-[1000] flex justify-end bg-slate-900/60 backdrop-blur-sm">
                 ...
                 >
                    <div className="p-6 md:p-8 border-b border-slate-100 flex justify-between items-center bg-slate-50">
                       <h2 className="text-xl font-extrabold text-slate-800">Identity Dossier</h2>
                    ...
                    <div className="flex-1 overflow-y-auto p-6 md:p-8 space-y-8 no-scrollbar bg-white">
                       ...
                       <div className="flex gap-6 items-center">
                          ...
                          <div>
                             ...
                             <div className="mt-3 flex gap-2">
                                <span className={cn("px-3 py-1 rounded-md text-xs font-extrabold uppercase tracking-widest border", getRoleColor(detailedUser.user.role))}>
                                   {detailedUser.user.role}
                                </span>
                                <span className={cn("px-3 py-1 rounded-md text-xs font-extrabold uppercase tracking-widest border", detailedUser.user.isActive ? "bg-emerald-50 text-emerald-600 border-emerald-200" : "bg-rose-50 text-rose-600 border-rose-200")}>
                       ...
                       {/* Metrics Grid */}
                       <div>
                          <h4 className="text-xs font-extrabold text-slate-400 uppercase tracking-widest mb-4">Platform Engagement Metrics</h4>
                          <div className="grid grid-cols-3 gap-4">
                             ...
                             <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100">
                                <p className="text-xs font-bold text-slate-500 uppercase tracking-widest mb-1">Questions</p>
                                <p className="text-2xl font-black text-slate-800 tabular-nums">{detailedUser.stats.questions || 0}</p>
                             </div>
                             <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100">
                                <p className="text-xs font-bold text-slate-500 uppercase tracking-widest mb-1">Answers</p>
                       ...
                       <div>
                          <h4 className="text-xs font-extrabold text-slate-400 uppercase tracking-widest mb-4">Onboarding Data</h4>
                       ...
                       {detailedUser.tutorProfile && (
                          <div className="bg-indigo-50 border border-indigo-100 rounded-2xl p-6 relative overflow-hidden">
                             ...
                             <h4 className="text-xs font-extrabold text-indigo-800 uppercase tracking-widest mb-4">Tutor Verification Data</h4>
