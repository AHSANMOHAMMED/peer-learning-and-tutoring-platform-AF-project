...
const seedTag = 'g6-allsubj-3med-v2';
...
async function getSeedAuthor() {
  ...
  if (!author) {
    author = await User.create({
      username: 'grade6_all_subjects_seed_admin',
      email: 'grade6.all.subjects.seed.admin@peerlearn.com',
...
async function seedGrade6AllSubjects3MediumsV2() {
  ...
  console.log(`Grade 6 all-subjects v2 seeding complete. Questions created: ${questionsCreated}, answers created: ${answersCreated}`);
  await mongoose.connection.close();
}

seedGrade6AllSubjects3MediumsV2().catch(async (error) => {
  console.error('Grade 6 all-subjects v2 seeding failed:', error.message);
  await mongoose.connection.close();
  process.exit(1);
});
